...
def remove_duplicates(
    ...
    Parameters:
    - ratings_df (pd.DataFrame): DataFrame with columns 'user_id', 'movie_id', 'timestamp'.

    Returns:
    - pd.DataFrame without duplicates
    """

    return ratings_df.loc[
        ratings_df.groupby(
            [
                "user_id",
                "movie_id",
...
def recommend_movies_knn(
    user_id: int, ratings_df: pd.DataFrame, k: int = 5, num_recommendations: int = 5
) -> list:
    """
    Recommends movies to a user using weighted k-NN (User-Based Collaborative Filtering).
...
def svd_model_fit(ratings_df: pd.DataFrame, **kwargs) -> list:
    """
...
def recommend_movies_svd(
    ...
    top_recommendations = sorted(predicted_ratings, key=lambda x: x[1], reverse=True)[
        :num_recommendations
    ]
